...
def save_mp3(tag, audio_file, frame_number, start_index, end_index, sr1=1000, sr2=44100):
    ...
    output_file = f"{frame_number}_{tag}.mp3"
...
frame_second = 10 # 프레임길이(초) 성능을 위해 10
...
for i in range(pre_start_idx, len(y1)- frame_size + 1, frame_size): # y1 을 frame 기준으로 순회 / step : frame_size >> 1로 하기?
    ...
    for j in range(0,len(y2) - frame_size + 1,1): # y2 를 step 1로 순회(전수조사)
    ...
    if(max(cosine_similarity)>save_sim_thresh):
        ...
        save_mp3("audio1",audio_file1, frame_num, src_start_idx, src_end_idx, sr1=sampling_rate, sr2=sampling_rate_final)

        # # 두 번째 음악 파일의 일부분을 MP3 파일로 저장
        save_mp3("audio2",audio_file2, frame_num, max_sim_start_idx, max_sim_end_idx, sr1=sampling_rate, sr2=sampling_rate_final)
